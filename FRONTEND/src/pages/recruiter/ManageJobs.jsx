...
function ManageJobs() {
  ...
  const [activeTab, setActiveTab] = useState(() =>
    window.location.pathname.includes("create") ? "create" : "jobs",
  );
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [{ data: jobsData }, { data: applicationsData }] =
          await Promise.all([jobsApi.list(), applicationsApi.list()]);
  ...
  const interviewApplicationIds = useMemo(
    () =>
      new Set(
        interviews
          .map(
            (interview) => interview.application?._id || interview.application,
          )
          .filter(Boolean),
      ),
    [interviews],
  ...
  const availableInterviewApplications = useMemo(
    () =>
      applications.filter(
        (application) =>
          application.status === "Shortlisted" &&
          !interviewApplicationIds.has(application._id),
      ),
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      setJobs((current) =>
        current.map((job) => (job._id === editingJobId ? data.job : job)),
      );
  ...
  const handleJobStatus = async (job, nextStatus) => {
    try {
      ...
      setJobs((current) =>
        current.map((item) => (item._id === job._id ? data.job : item)),
      );
      setSelectedJobId(job._id);
      setMessage(
        data.message ||
  ...
  const handleCreateJob = async (event) => {
    ...
    try {
      const { data } = await jobsApi.create({
        ...
        skillsRequired: createForm.skillsRequired
          .split(",")
          .map((item) => item.trim())
          .filter(Boolean),
      });
      ...
      setCreateMessage(
        error.response?.data?.message || "Could not create job.",
      );
    }
  };
  ...
  const handleScheduleInterview = async () => {
    if (!interviewForm.applicationId)
      return setInterviewFeedback({
        type: "error",
        text: "Please select an applicant.",
      });
    if (!interviewForm.scheduledAt)
      return setInterviewFeedback({
        type: "error",
        text: "Please choose an interview date and time.",
      });
  ...
  const getFilteredApplicants = (jobId) => {
    ...
    if (searchFilters.keyword) {
      ...
      apps = apps.filter(
        (a) =>
          a.user?.name?.toLowerCase().includes(kw) ||
    ...
    if (searchFilters.skills) {
      ...
      apps = apps.filter((a) =>
        (a.user?.skills || []).some((s) => s.toLowerCase().includes(sk)),
      );
    }
    if (searchFilters.experience) {
      const exp = searchFilters.experience.toLowerCase();
      apps = apps.filter((a) =>
        a.user?.experience?.toLowerCase().includes(exp),
      );
    }
    return apps;
  };
      ...
      <div className="max-w-6xl mx-auto py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div className="flex p-1 space-x-1 bg-gray-100/80 rounded-lg max-w-fit border border-gray-200">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "jobs" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-black ring-opacity-5" : "text-gray-600 hover:text-gray-900 hover:bg-gray-200/50"}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "interviews" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-black ring-opacity-5" : "text-gray-600 hover:text-gray-900 hover:bg-gray-200/50"}`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "create" ? "bg-white text-indigo-700 shadow-sm ring-1 ring-black ring-opacity-5" : "text-gray-600 hover:text-gray-900 hover:bg-gray-200/50"}`}
            >
        ...
        {activeTab === "jobs" ? (
          <div className="space-y-6">
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            ...
                            <span
                              className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${job.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-600 border-gray-200"}`}
                            >
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition shadow-sm"
                          >
                            Manage Job
                          </button>
                            ...
                            {openMenuId === job._id ? (
                              <div className="absolute right-0 top-full z-20 mt-1 w-48 rounded-lg border border-gray-200 bg-white shadow-lg py-1">
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleJobStatus(job, !job.isActive)
                                  }
                                  className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50"
                                >
            ...
            <div
              ref={detailsRef}
              className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 lg:p-8 mt-6"
            >
              {selectedJob ? (
                <div>
                  ...
                  <div className="flex border-b border-gray-200 mb-6 space-x-6">
                    <button
                      ...
                      className={`pb-3 text-sm font-medium transition-colors border-b-2 ${detailsTab === "info" ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
                    >
                      Job Info
                    </button>
                    <button
                      onClick={() => setDetailsTab("applicants")}
                      className={`pb-3 text-sm font-medium transition-colors border-b-2 flex items-center gap-2 ${detailsTab === "applicants" ? "border-indigo-600 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
                    >
                      Applicants{" "}
                      <span className="bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs">
                        {(applicantsByJob[selectedJob._id] || []).length}
                      </span>
                    </button>
                  </div>
                    ...
                    <div className="space-y-6 animate-in fade-in">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-900">
                            {new Date(
                              selectedJob.createdAt || Date.now(),
                            ).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {(selectedJob.skillsRequired || []).length ? (
                            selectedJob.skillsRequired.map((skill) => (
                              <span
                                key={skill}
                                className="px-3 py-1 bg-indigo-50 text-indigo-700 rounded-md text-sm font-medium border border-indigo-100"
                              >
                                {skill}
                              </span>
                            ))
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-white border border-gray-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:ring-2 focus:ring-indigo-600 outline-none"
                          ...
                          <select
                            className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-600 outline-none"
                      ...
                      <div className="border border-gray-200 rounded-xl">
                        {getFilteredApplicants(selectedJob._id).length ? (
                          <ul className="divide-y divide-gray-200 bg-white">
                            {getFilteredApplicants(selectedJob._id).map(
                              (application) => (
                                <li
                                  key={application._id}
                                  ...
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                      <div className="relative group">
                                        <button
                                          type="button"
                                          className="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm"
                                        >
                                          Update Status
                                        </button>
                ...
                <div className="py-8 text-center text-sm text-gray-500">
                  ...
                  Select "Manage Job" from a posting above to view its details
                  and applicants.
                </div>
              )}
            </div>
              ...
              {editingJobId ? (
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      className="px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-600 outline-none"
                      value={form.title}
                      onChange={(e) =>
                  ...
                  <div className="flex flex-col gap-1.5 md:col-span-2">
                    ...
                    <textarea
                      rows="2"
                      className="px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-600 outline-none resize-y"
                  ...
                  <div className="md:col-span-2 flex gap-3 pt-2">
                    ...
                    <button
                      type="button"
                      onClick={cancelEdit}
                      className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 shadow-sm"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <div className="py-8 text-center text-sm text-gray-500">
                  Choose "Edit Job" from a posting's menu to make changes.
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6 lg:p-8">
              <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Select a shortlisted candidate to arrange a meeting.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    {availableInterviewApplications.length ? (
                      availableInterviewApplications.map((app) => (
              ...
              <div className="mt-6 flex justify-end">
                <button
                  ...
                  className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSchedulingInterview
                    ? "Scheduling..."
                    : "Schedule Interview"}
              ...
              {interviewFeedback.text && (
                <div
                  className={`mt-4 p-3 rounded-lg text-sm font-medium border ${interviewFeedback.type === "error" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}
                >
                  {interviewFeedback.text}
                </div>
              )}
                  ...
                  {scheduledInterviews.map((interview) => (
                    ...
                    >
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500 mt-0.5">
                              {interview.job?.title || "Role Unspecified"}
                            </p>
                        ...
                        <div className="flex items-center gap-3">
                          {interview.location && (
                            <span className="px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-md text-xs font-medium text-gray-700">
                              {interview.location}
                            </span>
                          )}
              ...
              <form onSubmit={handleCreateJob} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      className="px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-600 outline-none"
                      placeholder={user?.company || "Company Ltd"}
                  ...
                  <div className="flex flex-col gap-1.5 md:col-span-2">
                    ...
                    <textarea
                      rows="6"
                      className="px-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-indigo-600 outline-none resize-y"
                      placeholder="Detail the role, expectations, and requirements..."
                ...
                <div className="pt-4 border-t border-gray-200 flex items-center justify-between">
                  {createMessage && (
                    <span className="text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-md">
                      {createMessage}
                    </span>
                  )}
                  <button
                    type="submit"
                    className="ml-auto px-8 py-3 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 shadow-sm"
                  >
                    Publish Job
                  </button>
                </div>
